...
import { TestingModuleAsync } from './src/TestingModuleAsync';

describe('app', () => {
  let app: NestExpressApplication;
  let appWithMiddleware: NestExpressApplication;
  let appWithFunctionConfigs: NestExpressApplication;
  let appAsync: NestExpressApplication;
  beforeAll(async () => {
    app = await NestFactory.create<NestExpressApplication>(TestingModule);
    appWithMiddleware = await NestFactory.create<NestExpressApplication>(
      TestingModuleWithMiddleware,
    );
    appWithFunctionConfigs = await NestFactory.create<NestExpressApplication>(
      TestingModuleWithConfigFunctions,
    );
    appAsync = await NestFactory.create<NestExpressApplication>(
      TestingModuleAsync,
    );
    await app.listen(3333);
    await appWithMiddleware.listen(3334);
    await appWithFunctionConfigs.listen(3335);
    await appAsync.listen(3336);
  });
  ...
  describe('universal decorator', () => {
    it('should test read user', async () => {
      const badRes = await request(app.getHttpServer()).get('/getName');
    ...
    it('should test write user', async () => {
      const badRes = await request(app.getHttpServer()).get('/writeName');
      expect(badRes.body.message).toBe('Forbidden resource');
  ...
  describe('Middleware app', () => {
    it('Testing permission with loaded user', async () => {
      const goodRes = await request(appWithMiddleware.getHttpServer()).get(
        '/getUser',
      );
  ...
  describe('Universal app with TestingModuleWithConfigFunctions', () => {
    it('should test read user', async () => {
      const badRes = await request(appWithFunctionConfigs.getHttpServer()).get(
        '/getName',
      );
      expect(badRes.body.message).toBe('Forbidden resource');
  ...
  describe('Universal app with Async registration', () => {
    it('should test read user', async () => {
      const badRes = await request(appAsync.getHttpServer()).get('/getName');
      expect(badRes.body.message).toBe('Forbidden resource');

      const goodRes = await request(appAsync.getHttpServer())
        .get('/getName')
        .set('x-user', 'readuser');
      expect(goodRes.text).toBe('Bob');
    });
    ...
    it('should test write user', async () => {
      const badRes = await request(appAsync.getHttpServer()).get('/writeName');
      expect(badRes.body.message).toBe('Forbidden resource');
      ...
      const badUserRes = await request(appAsync.getHttpServer())
        .get('/writeName')
        .set('x-user', 'baduser');
      expect(badUserRes.body.message).toBe('Forbidden resource');
    });
    ...
    it('should test read write user for multiple object age, name', async () => {
      ...
      const badRes = await request(appAsync.getHttpServer())
        .get('/writeNameAndAge')
        .set('x-user', 'readuser');
      expect(badRes.body.message).toBe('Forbidden resource');
    });
